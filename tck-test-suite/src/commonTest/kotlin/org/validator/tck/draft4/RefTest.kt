...
public class RefTest {
  ...
  public fun root_pointer_ref(): Unit {
    val schema = "{\"properties\":{\"foo\":{\"${'$'}ref\":\"#\"}},\"additionalProperties\":false}"
    val `match` = "{\"foo\":false}"
    run_test(schema, `match`, true, "DRAFT_4")
    val `recursive_match` = "{\"foo\":{\"foo\":false}}"
    run_test(schema, `recursive_match`, true, "DRAFT_4")
    val `mismatch` = "{\"bar\":false}"
    run_test(schema, `mismatch`, false, "DRAFT_4")
    val `recursive_mismatch` = "{\"foo\":{\"bar\":false}}"
    run_test(schema, `recursive_mismatch`, false, "DRAFT_4")
  }
  ...
  public fun relative_pointer_ref_to_object(): Unit {
    ...
    val `match` = "{\"bar\":3}"
    run_test(schema, `match`, true, "DRAFT_4")
    val `mismatch` = "{\"bar\":true}"
    run_test(schema, `mismatch`, false, "DRAFT_4")
  }

  @Test
  public fun relative_pointer_ref_to_array(): Unit {
    val schema = "{\"items\":[{\"type\":\"integer\"},{\"${'$'}ref\":\"#/items/0\"}]}"
    val `match_array` = "[1,2]"
    run_test(schema, `match_array`, true, "DRAFT_4")
    val `mismatch_array` = "[1,\"foo\"]"
    run_test(schema, `mismatch_array`, false, "DRAFT_4")
  }
  ...
  public fun escaped_pointer_ref(): Unit {
    ...
    val `slash_invalid` = "{\"slash\":\"aoeu\"}"
    run_test(schema, `slash_invalid`, false, "DRAFT_4")
    val `tilde_invalid` = "{\"tilde\":\"aoeu\"}"
    run_test(schema, `tilde_invalid`, false, "DRAFT_4")
    val `percent_invalid` = "{\"percent\":\"aoeu\"}"
    run_test(schema, `percent_invalid`, false, "DRAFT_4")
    val `slash_valid` = "{\"slash\":123}"
    run_test(schema, `slash_valid`, true, "DRAFT_4")
    val `tilde_valid` = "{\"tilde\":123}"
    run_test(schema, `tilde_valid`, true, "DRAFT_4")
    val `percent_valid` = "{\"percent\":123}"
    run_test(schema, `percent_valid`, true, "DRAFT_4")
  }
  ...
  public fun nested_refs(): Unit {
    ...
    run_test(schema, `nested_ref_valid`, true, "DRAFT_4")
    val `nested_ref_invalid` = "\"a\""
    run_test(schema, `nested_ref_invalid`, false, "DRAFT_4")
  }
  ...
  public fun ref_overrides_any_sibling_keywords(): Unit {
    ...
    val `ref_valid` = "{\"foo\":[]}"
    run_test(schema, `ref_valid`, true, "DRAFT_4")
    val `ref_valid__maxItems_ignored` = "{\"foo\":[1,2,3]}"
    run_test(schema, `ref_valid__maxItems_ignored`, true, "DRAFT_4")
    val `ref_invalid` = "{\"foo\":\"string\"}"
    run_test(schema, `ref_invalid`, false, "DRAFT_4")
  }

  @Test
  public fun `$ref_prevents_a_sibling_id_from_changing_the_base_uri`(): Unit {
    val schema =
        "{\"id\":\"http://localhost:1234/sibling_id/base/\",\"definitions\":{\"foo\":{\"id\":\"http://localhost:1234/sibling_id/foo.json\",\"minimum\":2},\"base_foo\":{\"${'$'}comment\":\"this canonical uri is http://localhost:1234/sibling_id/base/foo.json\",\"id\":\"foo.json\",\"minimum\":5}},\"allOf\":[{\"${'$'}comment\":\"${'$'}ref resolves to http://localhost:1234/sibling_id/base/foo.json, not ttp://localhost:1234/sibling_id/foo.json\",\"id\":\"http://localhost:1234/sibling_id/\",\"${'$'}ref\":\"foo.json\"}]}"
    val `$ref_resolves_to__definitions_foo__data_validates` = "10"
    run_test(schema, `$ref_resolves_to__definitions_foo__data_validates`, true, "DRAFT_4")
    val `$ref_resolves_to__definitions_foo__data_does_not_validate` = "1"
    run_test(schema, `$ref_resolves_to__definitions_foo__data_does_not_validate`, false, "DRAFT_4")
  }

  @Test
  public fun remote_ref__containing_refs_itself(): Unit {
    val schema = "{\"${'$'}ref\":\"http://json-schema.org/draft-04/schema#\"}"
    val `remote_ref_valid` = "{\"minLength\":1}"
    run_test(schema, `remote_ref_valid`, true, "DRAFT_4")
    val `remote_ref_invalid` = "{\"minLength\":-1}"
    run_test(schema, `remote_ref_invalid`, false, "DRAFT_4")
  }

  @Test
  public fun `property_named_$ref_that_is_not_a_reference`(): Unit {
    val schema = "{\"properties\":{\"${'$'}ref\":{\"type\":\"string\"}}}"
    val `property_named_$ref_valid` = "{\"${'$'}ref\":\"a\"}"
    run_test(schema, `property_named_$ref_valid`, true, "DRAFT_4")
    val `property_named_$ref_invalid` = "{\"${'$'}ref\":2}"
    run_test(schema, `property_named_$ref_invalid`, false, "DRAFT_4")
  }
  ...
  public fun Recursive_references_between_schemas(): Unit {
    ...
    run_test(schema, `valid_tree`, true, "DRAFT_4")
    val `invalid_tree` =
    ...
    run_test(schema, `invalid_tree`, false, "DRAFT_4")
  }
  ...
  public fun refs_with_quote(): Unit {
    ...
    val `object_with_numbers_is_valid` = "{\"foo\\\"bar\":1}"
    run_test(schema, `object_with_numbers_is_valid`, true, "DRAFT_4")
    val `object_with_strings_is_invalid` = "{\"foo\\\"bar\":\"1\"}"
  ...
  public fun `naive_replacement_of_$ref_with_its_destination_is_not_correct`(): Unit {
    ...
    run_test(schema, `do_not_evaluate_the_$ref_inside_the_enum__matching_any_string`, false,
        "DRAFT_4")
    val `match_the_enum_exactly` = "{\"${'$'}ref\":\"#/definitions/a_string\"}"
    run_test(schema, `match_the_enum_exactly`, true, "DRAFT_4")
  }
}
